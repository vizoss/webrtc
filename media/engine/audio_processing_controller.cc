...
AudioProcessingApplyResult ApplyAudioProcessingOptions(AudioProcessing *apm, AudioDeviceModule *adm,
  ...
  // Echo cancellation follows the caller's AudioOptions as-is, even in
  // stereo mode. AEC3 (the software path here, given
  // isMultiChannelRenderEnabled/isMultiChannelCaptureEnabled) already has
  // multi-channel-aware correlation handling, and unconditionally forcing
  // AEC off in stereo mode caused audible echo in acoustically-coupled
  // scenarios (e.g. speakerphone with an open mic) -- worse than any
  // imprecision AEC might have under stereo.
